...
// register.js ke submit listener ke andar ye check daalein
...
registerForm.addEventListener('submit', async (e) => {
    ...
    // 🛑 2. Hard Password Check (Regex)
    // Iska matlab: Min 8 char, 1 Uppercase, 1 Lowercase, 1 Number, 1 Special Char
    const strongPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
    
    if (!strongPassword.test(password)) {
        alert("Password too weak! It must be at least 8 characters long and include: 1 Uppercase, 1 Lowercase, 1 Number, and 1 Special Character (@$!%*?&).");
        return;
    }

    // Baaki aapka Device Check aur Firebase logic yahan se shuru hoga...
});